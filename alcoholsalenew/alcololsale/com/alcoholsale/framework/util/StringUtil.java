package com.alcoholsale.framework.util;
...
import com.alcoholsale.admin.framework.context.webcontext.ThreadContextHolder;
		...
		if (dbFileSize >= 1024.0D) {
			if (dbFileSize >= 1048576.0D) {
				if (dbFileSize >= 1.073741824E9D) {
					temp = df.format(dbFileSize / 1024.0D / 1024.0D / 1024.0D)
							+ " GB";
				} else {
		...
		if (!toTrim(str).equals("")) {
			if (str.indexOf('$', 0) > -1) {
				while (str.length() > 0) {
					if (str.indexOf('$', 0) > -1) {
						sReturn = sReturn
								+ str.subSequence(0, str.indexOf('$', 0));
						sReturn = sReturn + "\\$";
